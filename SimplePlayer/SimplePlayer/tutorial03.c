...
void packet_queue_init_03(PacketQueue *q) {
	memset(q, 0, sizeof(PacketQueue));
	q->mutex = SDL_CreateMutex();
	q->cond = SDL_CreateCond();
}
int packet_queue_put_03(PacketQueue *q, AVPacket *pkt) {
...
static int packet_queue_get_03(PacketQueue *q, AVPacket *pkt, int block)
{
	AVPacketList *pkt1;
	int ret;
...
int audio_decode_frame_03(AVCodecContext *aCodecCtx, uint8_t *audio_buf, int buf_size) {
	...
	for (;;) {
		...
		if (packet_queue_get_03(&audioq, &pkt, 1) < 0) {
			return -1;
		}
...
void audio_callback_03(void *userdata, Uint8 *stream, int len) {
...
int ch03_play_audio(const char *pFilePath) {
	...
	wanted_spec.callback = audio_callback_03;
